...
class AC():
        ...
        for epoch_i in range(1, config.epoch + 1):
            ...
            for batch_i, batch in enumerate(test_loader):
                ...
                with torch.no_grad():
                    ...
                    music_seq = music_seq_ori[:, config.ds_rate//music_relative_rate:]
            ...
            training_data = prepare_dataloader(music_seqs, \
                (dance_up_seqs, dance_down_seqs), \
                beat_seqs, self.config.batch_size, \
                self.config.structure_generate.block_size+1, \
                music_dance_ratio=config.ds_rate//music_relative_rate, \
            ...
            for batch_i, batch in enumerate(training_data):
                ...
                music_seq = music_seq.to(self.device)[:, self.config.ds_rate // self.config.music_relative_rate:] # music (1..29)
                ...
                with torch.no_grad():
                    gpt.eval()
                    output, actor_loss, entropy = gpt.module.actor(quants_actor_input, music_seq[:, :-1*(self.config.ds_rate // self.config.music_relative_rate)], quants_actor_output, reduction=False) # output dance 1...28
                    gpt.train()
                    gpt.module.freeze_drop()
    ...
    def _dir_setting(self):
        ...
        self.evaldir = None
        if hasattr(self.config, 'folder_suffix'):
            self.evaldir = os.path.join(self.expdir, "eval_" + self.config.folder_suffix)
        else:
...
def prepare_dataloader(music_data, dance_data, beat_data, batch_size, interval, music_dance_ratio, wav_padding):

    modaac = MoDaSeqAC(music_data, dance_data, beat_data, interval, music_dance_ratio, wav_padding)
