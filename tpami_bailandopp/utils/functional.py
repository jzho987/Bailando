...
def generate(model, src_aud, src_aud_pos, src_pos, src_pos_pos):
    ...
    with torch.no_grad():
        ...
        # attention: AIST++ input the first 2 sec as condition

        bsz, src_seq_len, _ = src_aud.size()
        _, _ , pose_dim = src_pos.size()

        # skip first 120 frames
...
def img2video(expdir, epoch, audio_path=None):
    ...
    for dance in tqdm(dance_names, desc='Generating Videos'):
        #pdb.set_trace()
        name = dance.split(".")[0]
        cmd = f"ffmpeg -r 30 -i {image_dir}/{dance}/frame%06d.png -vb 20M -vcodec mpeg4 -y {video_dir}/{name}.mp4 -loglevel quiet"
...
def write2motion(dances_rotmat, dance_names, config, expdir, epoch, rotmat):
    epoch = int(epoch)
    # print(len(dances))
    # print(len(dance_names))
    assert len(dances_rotmat) == len(dance_names),\
        "number of generated dance != number of dance_names"
    
    if not os.path.exists(os.path.join(expdir, "motion")):
        os.makedirs(os.path.join(expdir, "motion"))
        print(os.path.join(expdir, "motion"), flush=True)

    ep_path = os.path.join(expdir, "motion", f"ep{epoch:06d}")
    ...
    # print("Writing Json...")
    for i in tqdm(range(len(dances_rotmat)),desc='Generating Jsons'):

        # if rotmat:
        #     mat, trans = dances[i]
        #     pkl_data = {"pred_motion": mat, "pred_trans": trans}
        # else:
        np_dance = dances_rotmat[i]
        root = np_dance[:, :3]
        rotmat = np_dance[:, 3:].reshape([-1, 3, 3])
        rotmat = get_closest_rotmat(rotmat)
        smpl_poses = rotmat2aa(rotmat).reshape(-1, 24, 3)
        pkl_data = {"smpl_poses": smpl_poses, 'smpl_trans':root}

        dance_path = os.path.join(ep_path, dance_names[i] + '.pkl')
        with open(dance_path, 'wb') as handle:
...
def visualizeAndWrite(results,config,expdir,dance_names, epoch, quants=None):
    ...
    if config.data.name == "aist":
        ...
        for i in range(len(results)):
            ...
            # np_dance[:, :3] = np_dance[:, :3] - np_dance[:1, :3]
    ...
    if len(np_dances_rotmat):
        write2motion(np_dances_rotmat, dance_names,config.testing, expdir, epoch, rotmat=config.rotmat )
...
def load_data_aist(data_dir, interval=120, move=40, rotmat=False, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    ...
    return music_data_norm, dance_data, file_names
    # , [fn.replace('.json', '') for fn in fnames]


def load_data_aist_quantize_music(data_dir, interval=120, move=40, rotmat=False, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    tot = 0
    music_data, dance_data = [], []
    fnames = sorted(os.listdir(data_dir))
    # print(fnames)
            ...
            if interval is not None:
                ...
                for i in range(0, seq_len, move):
                    ...
                    if len(music_sub_seq) == interval_sample and len(dance_sub_seq) == interval:
                        ...
                        # print(external_wav_rate, len(music_sub_seq), len(dance_sub_seq), flush=True)
                        padding_sample = wav_padding // music_sample_rate
                        # Add paddings/context of music
                        music_sub_seq_pad = np.zeros((interval_sample + padding_sample * 2, ), dtype=music_sub_seq.dtype)
...
def load_test_data_aist(data_dir, rotmat, move, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    ...
    for fname in fnames:
        ...
        with open(path) as f:
            ...
            if external_wav is not None:
                # print('load from external wav!')
                wav_path = os.path.join(external_wav, fname.split('_')[-2] + '.json') if 'demo' not in data_dir else os.path.join(external_wav, fname)
                with open(wav_path) as ff:
                    sample_dict_wav = json.loads(ff.read())
            ...
            # print('Here!', wav_padding, flush=True)
            # zero padding left
...
def load_test_data_aist_quantize_music(data_dir, rotmat, move, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
...
def visualizeAndWritefromdata(pkl_root, target_pkl_root, config=None):
    if config is None:
        config = VSConfig()
    ...
    np_dances_original = []
    # dance_datas = []
    for (ii, pkl_name) in enumerate(dance_names):
        # if os.path.isdir(os.path.join(pkl_root, pkl_name)):
        #     continue
        # result = np.load(os.path.join(pkl_root, pkl_name), allow_pickle=True).item()['pred_position']
        # dance_names.append(pkl_name)
        result = dance_datas[ii]
        np_dance = result
        ...
        np_dance = np_dance.reshape([b, 24, 3])
        pkl_data = {"pred_position": np_dance}
        np.save(os.path.join(target_pkl_root, pkl_name), pkl_data)
    ...
    # write2pkl(dance_datas, dance_names, config.testing, expdir, epoch, rotmat=config.rotmat)
    write2json(np_dances, dance_names,config, target_pkl_root, 123221)
    visualize(config, dance_names, target_pkl_root, 123221, quants=None)
    img2video(target_pkl_root, 123221)
...
def visualizeAndWritefromPKL_edge(pkl_root, target_pkl_root, config=None):
    smpl = SMPL(model_path='/mnt/lustre/syli/dance/Bailando/smpl', gender='MALE', batch_size=1)
    ...
    for pkl_name in os.listdir(pkl_root):
        ...
        pkl_content = np.load(os.path.join(pkl_root, pkl_name), allow_pickle=True)
        smpl_poses = pkl_content['smpl_poses']
        root = pkl_content['smpl_trans']
        ...
        np_dance_clone = np_dance.copy()
        np_dance_clone[:, :, 1] = np_dance[:, :, 2]
        np_dance_clone[:, :, 2] = np_dance[:, :, 1]

        pkl_data = {"pred_position": np_dance_clone}
        np.save(os.path.join(target_pkl_root, pkl_name), pkl_data)

        result = np_dance_clone
        dance_names.append(pkl_name)
